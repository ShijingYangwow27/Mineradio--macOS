...
function pickReleaseAsset(assets) {
  ...
  if (process.platform === 'win32') {
    ...
    preferred = list.find(a => /\.(dmg|zip)$/i.test(a && a.name || ''))
      || list.find(a => /\.(pkg)$/i.test(a && a.name || ''))
      || list[0];
  } else {
    preferred = list.find(a => /\.(zip|7z|tar\.gz|pkg)$/i.test(a && a.name || ''))
      || list[0];
  }
...
function safeUpdateFileName(name, version) {
  let fallback = `Mineradio-${version || APP_VERSION}.exe`;
  if (process.platform === 'darwin') fallback = `Mineradio-${version || APP_VERSION}.zip`;
  else if (process.platform === 'linux') fallback = `Mineradio-${version || APP_VERSION}.zip`;
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn.startsWith('/tools/')) {
    serveStatic(res, path.join(__dirname, pn));
    return;
  }
