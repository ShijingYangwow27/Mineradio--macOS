...
> 当前版本：v1.1.5 |  平台：**macOS（Apple Silicon / Intel）**  |  许可证：**GPL-3.0**
...
- **新增听歌报告功能**：用户可查看自己的听歌数据，对标网易云音乐。
- **GPU 优化：背景媒体后台隐藏**：切到后台或最小化时自动暂停背景视频、隐藏图层以节省 GPU；背景媒体改用独立存储，关闭重开后仍在。
- **多行歌词与显示模式（主要更新）**：歌词支持单行 / 三行 / 沉浸三种显示模式，默认行数改为三行；用户存档预设可保存不同行数的布局；修复多行切单行时残留歌词、以及用户预设丢歌词的问题。
- **新增壁纸库（媒体背景库）**：上传的图片 / 视频统一存入本地库，可自由切换、预览与管理。支持拖拽 / 粘贴上传、上传自动应用、删除二次确认；缩略图自动生成，当前应用项高亮；「清除」仅取消当前应用、壁纸仍保留在库中。
- **评论卡片生命周期修复**：序列播放结束不再回跳到第一张（停在最末张，歌曲真正结束时才消失）；单曲循环重置后评论卡片稳定显示。
- **歌词动画优化**：移除色散效果，仅保留 float + glitch 两种风格，动画表现更干净。
